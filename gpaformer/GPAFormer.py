...
class PartialNoGradRG(nn.Module):
    def __init__(self, rg: nn.Module):
        super().__init__()
        self.rg = rg
    ...
    def forward(self, x):
        if x.is_cuda:
            with torch.amp.autocast('cuda', enabled=False):
                return self.rg(x.float())
        else:
            return self.rg(x.float())

    # delegate configuration calls to the wrapped RegionGrowing
    def set_config(self, *args, **kwargs):
        return self.rg.set_config(*args, **kwargs)
...
class SegFormer3D(nn.Module):
    def __init__(
        ...
        # decoder takes in the encoder's 3 outputs in reversed order (c3,c2,c1)
        decoder_in_dims = embed_dims[:3][::-1]
        self.segformer_decoder = SegFormerDecoderHead(
            input_feature_dims=decoder_in_dims,
...
#         x = self.segformer_decoder(c1, c2, c3, c4)
        x = self.segformer_decoder(c1, c2, c3)
        return x
...
class PatchEmbedding(nn.Module):
    def __init__(
        ...
        super().__init__()
        if stage==1:
            # Stage-1 uses two strided convs; ensure overall downsample x4 to match decoder x4 upsample
            assert stride == 2, "PatchEmbedding(stage=1) expects stride=2 so two convs yield /4"
...
class TransformerBlock(nn.Module):
    ...
    def forward(self, x, x_):
        x = x + self.attention(self.norm1(x), self.norm1(x_))
...
class MixVisionTransformer(nn.Module):
    def __init__(
        ...
        self.graph_combine_1 = RegionGrowing(
            in_channels=embed_dims[0],
            num_clusters=216,
            alpha=0.5
        )
        self.graph_combine_2 = RegionGrowing(
            in_channels=embed_dims[1],
            num_clusters=216,
            alpha=0.5
        )
        # stage-specific physical scales and defaults
        self.graph_combine_1.set_config(
            voxel_spacing=(6.0, 6.0, 8.0),
            sigma_mm=(12.0, 12.0, 16.0),
            chi2_thresh=11.34,
            combine="blend",
            feat_temp=0.5,
            neighbor_radius=1
        )
        self.graph_combine_2.set_config(
            voxel_spacing=(12.0, 12.0, 16.0),
            sigma_mm=(24.0, 24.0, 32.0),
            chi2_thresh=11.34,
            combine="blend",
            feat_temp=0.5,
            neighbor_radius=1
        )
        # wrap with PartialNoGradRG to disable autocast but keep gradients
        self.graph_combine_1 = PartialNoGradRG(self.graph_combine_1)
        self.graph_combine_2 = PartialNoGradRG(self.graph_combine_2)
...
#             nn.ReLU(inplace=True),           
#         )        
        self.norm_pe = nn.LayerNorm(embed_dims[0])
        self.attention = PESelfAttention(
...
#             qkv_bias=True
#         )
#         self.proj = nn.Linear(embed_dims[0] * 2, embed_dims[0])
#         self.mlp = nn.Sequential(
#             nn.LayerNorm(embed_dims[0] * 2),
#             nn.Linear(embed_dims[0] * 2, embed_dims[0] * 2),
#             nn.GELU(),
...
#             nn.Sigmoid()
#         )
        self.embed_1_1 = PatchEmbedding(
            in_channel=in_channels,
            embed_dim=embed_dims[0],
            kernel_size=patch_kernel_size[0][0],
            stride=patch_stride[0],
            padding=patch_padding[0][0],
            stage=1,
        )
        self.embed_1_2 = PatchEmbedding(
            in_channel=in_channels,
            embed_dim=embed_dims[0],
            kernel_size=patch_kernel_size[0][1],
            stride=patch_stride[0],
            padding=patch_padding[0][1],
            stage=1,
        )
        self.embed_1_3 = PatchEmbedding(
            in_channel=in_channels,
            embed_dim=embed_dims[0],
            kernel_size=patch_kernel_size[0][2],
            stride=patch_stride[0],
            padding=patch_padding[0][2],
            stage=1,
        )
...
#         self.norm4 = nn.LayerNorm(embed_dims[3])
    ...
    def forward(self, x):
        ...
        ori_x=x
        # stage 1
        x_1 = self.embed_1_1(x)
        x_2 = self.embed_1_2(x)
        x_3 = self.embed_1_3(x)

        x = x_1 + x_2 + x_3
        x = x + self.attention(self.norm_pe(x))

        B, N, C = x.shape
        n = cube_root(N)       
        for i, blk in enumerate(self.tf_block1):
            kv = self.graph_combine_1(x)
            x = blk(x, kv)
        x = self.norm1(x)
...
#             kv = self.graph_combine_3(x,ori_x)    
#             x = blk(x, kv)
            x = blk(x, x)
        x = self.norm3(x)
        # (B, N, C) -> (B, D, H, W, C) -> (B, C, D, H, W)
...
###################################################################################
def cube_root(n):
    n_int = round(math.pow(n, (1 / 3)))
    assert n_int ** 3 == n, f"Expected perfect cube token count, got N={n}"
    return n_int
...
#         )
        ...
        self.linear_fuse = nn.Sequential(
            nn.Conv3d(
                in_channels=3 * decoder_head_embedding_dim,
    ...
    def forward(self, c1, c2, c3):
        # Sanity checks: decoder expects reversed dims [c3,c2,c1]
        assert c3.shape[1] == self.linear_c3.proj.in_features, "Decoder expects c3 channels == input_feature_dims[0]"
        assert c2.shape[1] == self.linear_c2.proj.in_features, "Decoder expects c2 channels == input_feature_dims[1]"
        assert c1.shape[1] == self.linear_c1.proj.in_features, "Decoder expects c1 channels == input_feature_dims[2]"
